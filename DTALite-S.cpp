...
void g_TrafficSimulation()
{
	...
	for (int l = 0; l < g_number_of_links; l++)
	{
		...
		if (g_link_vector[l].m_End_of_RedTime_in_simu_interval==0)
		{
...
#pragma omp critical
								...
								if (g_link_vector[link_seq_no].link_flag==0)
								{
...
int g_AssignThreadsBasedonOriginNodes()
{
	...
	for (int i = 0; i < number_of_threads; i++)
	{
		...
		pNetworkForSP[i].AllocateMemory(g_number_of_nodes, g_number_of_links,time_index);
	}
...
void g_OutputFiles()
{
	...
	else
	{
		...
		{
			CAgent* p_agent = &(g_agent_vector[a]);
			float TA_in_min= p_agent->m_Veh_LinkArrivalTime_in_simu_interval[0] * g_number_of_seconds_per_interval / 60.0;
			float TD_in_min = p_agent->m_Veh_LinkArrivalTime_in_simu_interval[p_agent->path_link_seq_no_vector.size() - 1] * g_number_of_seconds_per_interval / 60.0;
			float cost = TD_in_min - TA_in_min;
			float path_cost;
...
public:
			...
			for (int t = 0; t < g_number_of_optimization_time_intervals; t++)
			{
				if (g_link_vector[link_no].service_type == enum_road_capacity_link_type)
				{
					...
					g_link_vector[link_no].time_dependent_ADMM_multiplier_vector[t] = max(0,
						(g_penalty_RHO / 2.0)*(2 * time_dependent_visit_counts_used_by_other_vehicles - 2*g_link_vector[link_no].time_depedent_capacity_vector[t]+1));
	...
	float optimal_time_dependenet_dynamic_programming(int origin_node,int departure_time_beginning,int arrival_time_ending,int vehicle_capacity,int optimization_method,int BestKSize)
		// time-dependent label correcting algorithm with double queue implementation
	{
...
void reset_link_cost()
{
	for (int l = 0; l < g_number_of_links; l++)
	{
		for (int t = 0;t < time_index;t++)
		{
			g_link_vector[l].time_dependent_link_cost[t] = g_link_vector[l].link_cost;
		}
	}
}
	...
	for (int a = 0; a < g_agent_vector.size(); a++)
	{
		for (int l = 0; l < g_number_of_links; l++)
		{
			for (int t = 0;t < time_index;t++)
			{
				...
				//cout << "路段" << l << "在时间" << t << "上的费用为" << g_link_vector[l].time_dependent_link_cost[t] << endl;
}
		}
...
void find_sp_for_each_agent_use_ADMM(int mu, int current_a)
{
	g_pNetworkForSP = new NetworkForSP; // create n copies of network, each for a subset of agents to use	
	g_pNetworkForSP->AllocateMemory(g_number_of_nodes, g_number_of_links,time_index);
	for (int l = 0; l < g_number_of_links; l++)
	{
		for (int t = 0;t < time_index;t++)
		{
			g_pNetworkForSP->m_link_cost_array[l][t] = g_link_vector[l].free_flow_travel_time_in_min + g_link_vector[l].time_dependent_LR_multiplier_vector[t] + g_penalty_RHO / 2 + g_penalty_PHO*(mu - 1);
		}
	}
	g_pNetworkForSP->optimal_label_correcting(g_agent_vector[current_a].origin_node_id - 1, g_agent_vector[current_a].destination_node_id - 1, g_agent_vector[current_a].departure_time_in_min, g_agent_vector[current_a].arrival_time_in_min, current_a);
}
	...
	for (int k = 1; k < LR_iteration_no; k++)
	{
		...
		for (int l = 0;l < g_link_vector.size();l++)
		{
			for (int t = 0;t < time_index;t++)
			{
				...
				//cout << "路段" << l << "在时间" << t << "上的拉格朗日乘子为" << g_link_vector[l].time_dependent_LR_multiplier_vector[t] << endl;
			}
		}
		//ADMM process
		//set mu
		...
		for (int a = 0; a < g_agent_vector.size(); a++)
		{
			for (int l = 0;l < g_link_vector.size()-2;l++)//减2是不更新起终点的费用，防止出现负数
			{
				for (int t = 0;t <time_index;t++)
				{
					int mu = 0;
					for (int a1 = 0;a1 < g_agent_vector.size();a1++)
					{
					 ...
					 //cout << "对于出行者" << a << "路段" << l << "在出发时间" << t << "上的费用为" << g_link_vector[l].time_dependent_link_cost[t] << endl;
...
std::vector<information> CA_vector;
...
void Cumulative_Curve_Based_DP()
{
	...
	cumulative_vector[0].m_CA_vector[0].current_no = 0;
	cumulative_vector[0].m_CA_vector[0].pred_CA_positon = 0;
	cumulative_vector[0].m_CA_vector[0].queue_length = 0;
	cumulative_vector[0].m_CA_vector[0].cost = 0;
	cumulative_vector[0].m_CA_vector[0].value = 0;
	cumulative_vector[0].m_CA_vector[0].CD_value = 0;


	for (int t = 1;t < g_wished_inflow_vector.size();t++)// for each t
	{
		//for each last CA
		for (int size = 0; size < cumulative_vector[t - 1].m_CA_vector.size(); size++)
		{
			// for each inflow rate
			for (int current_inflow = 1;current_inflow < max_inflow + 1; current_inflow++)
			{
				int next_A = min(cumulative_vector[t - 1].m_CA_vector[size].value + current_inflow, no_of_vehicle);
				int next_outflow = min(max_cap, cumulative_vector[t - 1].m_CA_vector[size].queue_length + current_inflow);
				int next_D = min(no_of_vehicle, cumulative_vector[t - 1].m_CA_vector[size].CD_value + next_outflow);
				int current_queue = next_A - next_D;
				...
				int current_cost = current_queue + cumulative_vector[t - 1].m_CA_vector[size].cost + schedule_delay;
				// update
				int flag = 0;
				for (int compare = 0; compare < cumulative_vector[t].m_CA_vector.size(); compare++)
				{
					if (next_A == cumulative_vector[t].m_CA_vector[compare].value && current_cost < cumulative_vector[t].m_CA_vector[compare].cost)
					{
						cumulative_vector[t].m_CA_vector[compare].pred_CA_positon = cumulative_vector[t - 1].m_CA_vector[size].current_no;
						cumulative_vector[t].m_CA_vector[compare].CD_value = next_D;
						cumulative_vector[t].m_CA_vector[compare].cost = current_cost;
						cumulative_vector[t].m_CA_vector[compare].queue_length = current_queue;
						flag++;
					}
					else if (next_A == cumulative_vector[t].m_CA_vector[compare].value)
					{
						flag++;
					}
				}
				// flag == 0 -> not update, but add a new CA
				if (flag == 0)
				{
					information information;
					information.value = next_A;
					information.CD_value = next_D;
					information.queue_length = next_A - next_D;
					information.cost = current_cost;
					information.pred_CA_positon = cumulative_vector[t - 1].m_CA_vector[size].current_no;
					information.current_no = cumulative_vector[t].m_CA_vector.size();
					cumulative_vector[t].m_CA_vector.push_back(information);
				}
			}
		}
	}
